...
S2 = s2+2*sh2;
hLarge = zeros(S1,S2);
hLarge(round((S1-sh1)/2.)+1:round((S1+sh1)/2.),round((S2-sh2)/2.)+1:round((S2+sh2)/2.)) = h;
%hFFT   = fftshift(hLarge);
hFFT   = fft2(circshift(hLarge,[round(S1/2) round(S2/2)]-1));
outFFT = fft2(out);
out    = ifft2(outFFT.*hFFT);
%out=filter2(h,out);

%figure(16);imagesc(out);colorbar;
out = real(out(ibl:ibr,jbl:jbr));
end
