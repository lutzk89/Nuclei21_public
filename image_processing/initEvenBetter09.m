...
% afilename='/usr/scratch2/ulrich/DrosophilaEmbryo/better_data/d25.tif';
% outfolder='/usr/scratch2/ulrich/DrosophilaEmbryo/better_data/d25/';
...
afilename   = ['/usr/scratch2/ulrich/DrosophilaEmbryo/better_data/' rawfilename '.tif'];
outfolder   = ['/usr/scratch2/ulrich/DrosophilaEmbryo/better_data/' rawfilename '/'];
disp(' ');
...
%repaintValue = 78.;
%d14:
%[XXX YYY]=ndgrid(1:fileInfo(1).Height,1:fileInfo(1).Width); repaintIndices=find((XXX-530).^2+(YYY-1125).^2<400); clear XXX YYY;
%repaintValue = 51.;
...
set(1,'Position',[10 630 size(imSav0,2)/2. size(imSav0,1)/2.])
...
h = bp_sample(low_limit, high_limit, 450);%-0.4*bp_sample(0.0,.01,100);
%h2=bp_sample(0,.4,25);

im=filter2fft(h,imSav);
%im=cutoff(im,0.,2*sqrt(mean(mean(im.^2)) - mean(mean(im))^2));
figure(5);imagesc(im);colorbar
set(5,'Position',[610 320 size(imSav0,2)/2. size(imSav0,1)/2.])
